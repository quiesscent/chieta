...
const UserDashboard = () => {
  ...
  const [availableDesks, setAvailableDesks] = useState(openFloorDesks.filter(d => d.status === 'available'));
  const [unavailableDesks, setUnavailableDesks] = useState(openFloorDesks.filter(d => d.status === 'unavailable'));
  const [bookedDesks, setBookedDesks] = useState(openFloorDesks.filter(d => d.status === 'booked'));
  ...
  const handleBookingConfirm = (deskId: string, date: string, time: string, type: 'desk' | 'office') => {
    ...
    setDeskBookings(prev => ({ ...prev, [deskId]: { user: userName, status: 'booked' } }));
    setAvailableDesks(prev => prev.filter(d => d.id !== deskId));
    setBookedDesks(prev => [...prev, openFloorDesks.find(d => d.id === deskId) || { id: deskId, type: type, status: 'booked' }]);
  ...
  const handleBookingCancel = (bookingId: string) => {
    ...
    if (booking && bookedDesks.find(d => d.id === booking.deskId)) {
      ...
      setAvailableDesks(prev => [...prev, openFloorDesks.find(d => d.id === booking.deskId) || { id: booking.deskId, type: booking.type, status: 'available' }]);
    }
