...
const prepareDeskData = (desks: any[]) => desks.map(desk => ({
  'Desk ID': desk.id,
  'Type': desk.type,
  'Status': desk.status,
...
const prepareLogData = (logs: any[]) => logs.map(log => ({
  'Log ID': log.id,
  'User': log.user,
  'Action': log.action,
  'Desk': log.desk,
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div className="mb-4 flex justify-end">
          <Button onClick={handleDownloadAllData} variant="outline" className="flex items-center gap-2">
        ...
        <Card className="mt-8 shadow-custom-md">
          ...
          <CardContent>
            {/* Desk grid with 180-degree rotation to align horizontally */}
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-2 sm:gap-3 lg:gap-4">
              {desks.map((desk) => (
                <div key={desk.id} className="text-center">
                  ...
                  >
                    <span className="text-xs sm:text-xs" style={{ transform: 'rotate(180deg)' }}>
                      {desk.id.split('-')[1]}
                    </span>
                  </div>
                  <Button
