...
import { updateDeskStatus } from "@/services/apiClient";
...
export const DeskManagementModal = ({
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      const update = await updateDeskStatus(Number(desk?.id), data);
    } catch (err) {
      toast({
        title: "Update Failed",
      ...
      <DialogContent className="sm:max-w-md animate-scale-in">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2 text-primary">
            ...
            <span>
              Manage {desk.type === "office" ? "Office" : "Desk"} {desk.id}
            </span>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            {selectedStatus && selectedStatus !== desk.status && (
              <div className="text-sm text-muted-foreground">
                Status will be changed from <strong>{desk.status}</strong> to{" "}
          ...
          <div className="bg-muted/20 rounded-lg p-3 text-xs text-muted-foreground">
            ...
            <ul className="space-y-1">
              ...
              <li>
                <strong>Unavailable:</strong> Temporarily out of service
              </li>
              <li>
                <strong>Inactive:</strong> Permanently disabled
              </li>
              <li>
                <strong>Booked:</strong> Reserved by a user
              </li>
              <li>
                <strong>Checked In:</strong> Currently occupied
              </li>
            </ul>
          </div>
